...
QUINTE_RESULT_VERSION = "2.1"
QUINTE_MANIFEST_VERSION = "2.0"
QUINTE_PROTOCOL_VERSION = "1.0"
QUINTE_TRIAL_MANIFEST_VERSION = "1.0"
QUINTE_CLI_ENVELOPE_VERSION = "1.0"
QUINTE_HOST_RECEIPT_VERSION = "1.0"
QUINTE_HOST_RECEIPT_OPERATIONS = frozenset({"inspect", "reconcile"})
MAGI_PRODUCT_VERSION = "1.0"
AUTHORIZATION_VERSION = "1.0"
AUTHORIZATION_CONSUMPTION_VERSION = "1.0"
ROUTE_EXECUTION_REPORT_VERSION = "2.0"
AUTHORIZATION_MAX_LIFETIME = timedelta(hours=8)
...
def validate_authorization_artifact(
    ...
    *,
    ...
    """Validate a short-lived, action-bound user authorization."""
    ...
    if unknown:
        errors.append(f"authorization has unknown fields: {', '.join(unknown)}")
    if missing:
        errors.append(f"authorization is missing fields: {', '.join(missing)}")
    if artifact.get("authorization_version") != AUTHORIZATION_VERSION:
        errors.append("authorization_version is unsupported")
    if not isinstance(artifact.get("authorization_id"), str) or not artifact["authorization_id"].strip():
        errors.append("authorization_id must be a non-empty string")
    if artifact.get("authorized_by") != "user":
        errors.append("authorized_by must be user")
    if artifact.get("decision") != "authorize":
        errors.append("authorization decision must be authorize")
    expected_binding = action_binding_sha256(request)
    if artifact.get("action_binding_sha256") != expected_binding:
        errors.append("authorization action binding does not match the route request")
    if artifact.get("action_scope") != request.get("action_scope"):
        errors.append("authorization action scope does not match the route request")
    issued_at = parse_utc_timestamp(artifact.get("issued_at"))
    expires_at = parse_utc_timestamp(artifact.get("expires_at"))
    if issued_at is None:
        errors.append("issued_at must be an RFC 3339 UTC timestamp")
    if expires_at is None:
        errors.append("expires_at must be an RFC 3339 UTC timestamp")
    if issued_at is not None and expires_at is not None:
        if expires_at <= issued_at:
            errors.append("expires_at must be after issued_at")
        if expires_at - issued_at > AUTHORIZATION_MAX_LIFETIME:
            errors.append("authorization lifetime exceeds eight hours")
        current = now or datetime.now(timezone.utc)
        if issued_at > current + timedelta(minutes=5):
            errors.append("authorization is issued in the future")
        if expires_at <= current:
            errors.append("authorization has expired")
    return errors


def summarize_authorization_artifact(
    ref: str | None,
    request: dict[str, Any],
    *,
    ...
    try:
        ...
        errors.extend(validate_authorization_artifact(parsed, request))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        errors.append(f"authorization cannot be read: {exc}")
    return {
        "required": required,
