#!/usr/bin/env python3
"""Validate HIGHBALL route pairing reports."""
...
def load_report(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError("route pairing report must be a JSON object")
    return value
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate a HIGHBALL route pairing report")
    ...
    try:
        ...
        if not errors:
            expected = BUILDER.expected_report(args.report_file.resolve(), report)
            if report != expected:
    ...
    if recommendation in {"prefer_baseline", "block_target"}:
        print(f"[HIGHBALL] route pairing report valid; recommendation is {recommendation}", file=sys.stderr)
        return 1

    print(f"[HIGHBALL] route pairing report valid; recommendation is {recommendation}")
    return 0
