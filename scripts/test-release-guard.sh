...
printf '{"packages":[{"name":"highball","version":"%s"}]}\n' "${FAKE_CARGO_VERSION:-${FIXTURE_VERSION:?}}"
EOF
...
run_guard() {
  (
    ...
    env \
      PATH="$tmp/bin:$PATH" \
      GITHUB_REPOSITORY=owner/highball \
...
expect_output() {
  ...
  if ! grep -qF "$pattern" "$tmp/command-output"; then
    printf 'not ok - %s\n' "$name" >&2
    cat "$tmp/command-output" >&2
    exit 1
  fi
...
reset_api
printf '[{"total_count":1,"workflow_runs":[{"id":699,"head_sha":"%s","display_title":"Release v%s from %s","event":"workflow_dispatch"}]}]\n' "$other_sha" "$burned_version" "$other_sha" >"$tmp/api/runs.json"
expect_pass 'well-formed historical attempt for a lower version passes' run_guard

reset_api
printf '[{"total_count":1,"workflow_runs":[{"id":699,"head_sha":"%s","display_title":"v%s","event":"workflow_dispatch"}]}]\n' "$other_sha" "$burned_version" >"$tmp/api/runs.json"
expect_pass 'malformed historical title for another version only warns' run_guard
expect_output 'warning names the skipped title' "ignoring unrecognized display_title: v$burned_version"

reset_api
printf '[{"total_count":1,"workflow_runs":[{"id":699,"head_sha":"%s","display_title":"v%s","event":"workflow_dispatch"}]}]\n' "$other_sha" "$fixture_version" >"$tmp/api/runs.json"
expect_fail 'malformed historical title naming the requested version fails' run_guard

reset_api
printf '[{"total_count":1,"workflow_runs":[{"id":699,"head_sha":"%s","display_title":"retry of %s","event":"workflow_dispatch"}]}]\n' "$other_sha" "$candidate" >"$tmp/api/runs.json"
expect_fail 'malformed historical title naming the candidate fails' run_guard

reset_api
printf '[{"total_count":1,"workflow_runs":["oops"]}]\n' >"$tmp/api/runs.json"
expect_fail 'Actions entry that errors jq fails closed' run_guard
expect_output 'jq failure is reported as a guard failure' 'candidate-reuse check failed'
...
mkdir "$asset_dir"
assets=(
  highball-aarch64-apple-darwin.tar.gz
  highball-aarch64-unknown-linux-gnu.tar.gz
  highball-x86_64-apple-darwin.tar.gz
  highball-x86_64-unknown-linux-gnu.tar.gz
)
...
printf 'extra\n' >"$asset_dir/highball-extra.tar.gz"
expect_fail 'extra archive fails' "$guard" --assets "$asset_dir"
rm "$asset_dir/highball-extra.tar.gz"
(
  cd "$asset_dir"
  LC_ALL=C sha256sum "${assets[@]}" >"$tmp/SHA256SUMS.full"
)
cp "$tmp/SHA256SUMS.full" "$asset_dir/SHA256SUMS"
expect_pass 'five-file checksummed set passes' "$guard" --assets "$asset_dir" --checksums
head -3 "$tmp/SHA256SUMS.full" >"$asset_dir/SHA256SUMS"
expect_fail 'checksum set binding only three archives fails' "$guard" --assets "$asset_dir" --checksums
sed -n '1p;2p;2p;3p' "$tmp/SHA256SUMS.full" >"$asset_dir/SHA256SUMS"
expect_fail 'duplicate checksum binding fails' "$guard" --assets "$asset_dir" --checksums
cp "$tmp/SHA256SUMS.full" "$asset_dir/SHA256SUMS"
