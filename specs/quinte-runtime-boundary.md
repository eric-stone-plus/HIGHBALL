...
request + residual trace
          │
          ▼
HIGHBALL chooses QUINTE (or MAGI)
          │  host receipt / completed product only
          ▼
QUINTE owns all internal execution and lifecycle
```

The QUINTE provider may currently be MiMo, but that is a runtime binding. It is
not a HIGHBALL seat, entitlement, quota, or claim that the product is
permanently tied to `mimo-v2.5-pro`. HIGHBALL must validate the binding recorded
in the QUINTE policy/manifest and must not derive it from prose, command names,
or an environment variable copied into an action packet.
...
MiMo transport parsing is QUINTE-owned. A stream that contains a valid earlier
