...
This matters for QUINTE. A trace may be structurally valid while
...
does not read QUINTE internal phase, lane, agent, retry, or pacing records. This keeps
...
completed QUINTE product counts as `complete` for route reliability, while a
non-`pass` packet-level `action_decision` remains `block`. Reports must not
relabel that completed product as an invocation failure or treat completion as
permission.
...
- QUINTE owns and validates its internal execution state.
